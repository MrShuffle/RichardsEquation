...
class L_scheme_Parallell():
    ...
    def error_on_element(self,element_num):
        ...
        R_h = np.dot((psi_local2-psi_local).T.reshape(len(psi_local)),Phi.T)
        
        
        val =0
        for k in range(len(Phi)):
            val += a[k][2]*self.L*R_h[k]**2+a[k][2]*np.linalg.norm(local_vals.K_in_Q[k]*a[k][2]*dPhi[k][:]@J_inv.T)**2*det_J        
...
class Newton_scheme_Parallell():
    ...
    def error_on_element(self,element_num):
        ...
        val =0
        for k in range(len(Phi)):
            val += a[k][2]*local_vals.theta_prime_Q[k]*R_h[k]**2+a[k][2]*np.linalg.norm(local_vals.K_in_Q[k]*a[k][2]*dPhi[k][:]@J_inv.T)**2*det_J        
