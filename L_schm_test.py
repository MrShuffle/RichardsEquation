# -*- coding: utf-8 -*-
"""
Created on Mon Oct 31 10:12:44 2022
...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 24 19:10:06 2022
...
def K_sp(thetaa):
    val = ((k_abs)*((thetaa-the_r)/(the_s-the_r))**(1/2))*(1-(sp.functions.elementary.complexes.Abs(1-(sp.functions.elementary.complexes.Abs((thetaa-the_r)/(the_s-the_r)))**exp_1))**exp_2)**2
...
if __name__ == '__main__':
    ...
    # _s = scheme.estimate_eta(psi_t)
    # cd = scheme.estimate_CN(K,K_prime,theta_prime,psi_t)
    ...
    for j in range(timesteps):
        ...
        while True:
            ...
            lhs = scheme.lhs
            rhs = scheme.rhs
            print(timer()-timeassembly ,'time assembly')
            ...
            print(timer()-lintime,'time error')
            valstop=scheme.linear_norm
            # Lmet.linearization_error(psi,psi)
            # print(Lmet.linear_norm,'bo')
            r_L = np.linalg.norm(psi-psi_k)
            #print(r_L,'L scheme 2norm')
            L_count +=1
            testL[count]=r_L
            
            Rconv = np.abs(np.log(r_L))**(1/L_count)
            # Lmet.L_to_N_eta(psi, psi_k, K_prime, theta_prime)
            # print(Lmet.eta_LtoN,'eta L to N')
            # print(Lmet.eta_LtoL,'eta L to L')
            # # if L_count >=2:
            #     Rconv = np.abs(np.log(testL[count])/np.log(testL[count-1]))**(1/(count))
            psi_L_old = psi
            #valstop= Lmet.linear_norm
            #if a<=1:
                
            # if Lmet.eta_LtoL>=1:
            #     Lmet.update_L(L_theta)
            #     print(Lmet.L,'L')
            # if Lmet.eta_LtoN<1.5:
            #     Switch= True
            # else:
            #     Switch = False
            # if L_count >= 3:
            #     rate = np.log(testL[count]/testL[count-1])/np.log(testL[count-1]/testL[count-2])
            #     #Rconv = np.abs(np.log(testL[count])/np.log(testL[count-1]))**(1/L_count)
            #     print(rate,'rate')
            # print(Rconv,'Rconv')  
