...
class LN_alg:
    def __init__(
        ...
        """
        d.Newton_method(K_np, theta_np, K_prime_np, theta_prime_np, f)
    ...
    def local_Newtonscheme_assembly(self, element_num):
        ...
        for l in range(len(Phi)):
            ...
            local_perm += (
                local_vals.K_in_Q[l] * a[l][2] * (dPhi[l][:] @ transform @ dPhi[l][:].T)
            )
            local_J_perm += (
    ...
    def local_Lscheme_assembly(self, element_num):
        ...
        local_gravity = np.zeros((P_El.num_dofs, 1))
        local_saturation_k = np.zeros((P_El.num_dofs, 1))
        local_saturation_k_ = np.zeros((P_El.num_dofs, 1))
        ...
        for l in range(len(Phi)):
            ...
            local_saturation_k_ += local_vals.theta_in_Q[l] * a[l, 2] * Phi[l][:].reshape(-1, 1)
            local_perm += (
                local_vals.K_in_Q[l] * a[l][2] * (dPhi[l][:] @ transform @ dPhi[l][:].T)
            )
            local_gravity += (
        ...
        print(local_saturation_k)
        print(local_saturation_k_)
        print()
    ...
    def estimate_CN(self, u):
        ...
        print("len(u)", len(u))
        theta_in_Q = self.theta_np(u)
        K_in_Q = self.K_np(theta_in_Q)
        theta_prime_Q = self.theta_prime_np(u)
