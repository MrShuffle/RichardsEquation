    ...
    def local_mass_assembly(self, element_num):
        ...
        for j in range(P_El.num_dofs):
            for i in range(P_El.num_dofs):
                valalt = np.sum(
                    np.multiply(1 / 2 * np.multiply(a[:, 2], Phi[:][i]), Phi[:][j])
                )
        ...
        for j in range(3):
            ...
            for k in range(3):
                ...
                val2 += (
                    local_vals.K_in_Q[k]
                    * a[k][2]
                    * np.inner(np.array([0, 1]), dPhi[k][j] @ J_inv.T)
                )
            ...
            for i in range(3):
                ...
                for l in range(len(Phi)):
                    ...
                    val5 += (
                        ...
                        * a[l][2]
                        * Phi[l][i]
                        * np.inner(np.array([0, 1]), dPhi[l][j] @ J_inv.T)
                    )
                    val6 += (
                        local_vals.theta_prime_Q[l] * a[l][2] * Phi[l][i] * Phi[l][j]
                    )
...
class L_scheme_Parallell:
    ...
    def error_on_element(self, element_num):
        ...
        local_vals = localelement_function_evaluation_Lnorm(
            self.d.K, self.d.theta, psi_local, psi_local2, P_El
        )
        R_h = np.dot((psi_local).T.reshape(len(psi_local)), Phi.T)
        ...
        for k in range(len(Phi)):
            val += (
                0.5 * a[k][2] * self.L * R_h[k] ** 2 * det_J
                + self.dt
                * 0.5
                * a[k][2]
                * local_vals.K_in_Q2[k]
...
class ML_scheme_Parallell:
    ...
    def local_MLscheme_assembly(self, element_num):
        ...
        # Local pressure head values
        psi_local = np.array([self.psi_k[cn[0]], self.psi_k[cn[1]], self.psi_k[cn[2]]])
        local_vals = localelement_function_evaluation_P(
            self.d.K, self.d.theta, self.d.theta_prime, psi_local, P_El
        )
        ...
        for j in range(P_El.num_dofs):
            ...
            for i in range(P_El.num_dofs):
                ...
                for l in range(len(Phi)):
                    L = np.amax(
                        [
                            local_vals.theta_prime_Q[l] + self.dt * self.m,
                            self.dt * 2 * self.m,
                        ]
                    )
    ...
    def error_on_element(self, element_num):
        ...
        for k in range(len(Phi)):
            val += (
                self.d.quad_weights[k]
                * (local_vals.theta_prime_Q2[k] + self.m)
                * R_h[k] ** 2
                * det_J
                + self.d.quad_weights[k]
                * self.dt
