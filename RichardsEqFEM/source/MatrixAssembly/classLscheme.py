# -*- coding: utf-8 -*-
"""
Created on Sat Sep 10 13:04:48 2022
...
class L_scheme():
    ...
    def update_at_iteration(self,psi_k):
        ...
        for e in range(self.g.num_cells):
            ...
            [J2, c2, J_inv] = reference_to_local(e,P_El.corners,cn)
            jac = np.linalg.det(J2)
            ...
            for j in range(P_El.num_dofs):
                ...
                for k in range(len(dPhi)):
                    ...
                    val2 += local_vals.K_in_Q[k]*w_i*np.inner(gravity,dPhi[k][j]@J_inv.T)
            ...
            for j in range(P_El.num_dofs):
                for i in range(P_El.num_dofs):
                    ...
                    for l in range(len(Phi)):
                        val3 += local_vals.K_in_Q[l]*quadrature[l][2]*dPhi[l][i]@transform@dPhi[l][j].T
            ...
            for j in range(P_El.num_dofs):
                for i in range(P_El.num_dofs):
                    ...
                    for k in range(len(Phi)):
                        ...
                        val4 += local_vals.K_prime_Q[k]*w_i*Phi[k][i]*local_vals.valgrad_Q[k]@transform@dPhi[k][j].T  
                        val3 += local_vals.K_in_Q[k]*w_i*dPhi[k][i]@transform@dPhi[k][j].T
                        
                        vec = [(quadrature_pt)[0][k],(quadrature_pt)[1][k]]
                        q_i = J2@vec +c2 # transformed quadrature points
                        w_i = quadrature[k][2] # weights
                     
                        val6 += local_vals.K_prime_Q[k]*w_i*Phi[k][i]*np.inner(gravity,dPhi[k][j]@J_inv.T)
...
class Picard_scheme():
    ...
    def __init__(self,dt,d,g,order,psi, K,theta,theta_prime,f):
    ...
    def update_at_iteration(self,psi_k):
        ...
        for e in range(self.g.num_cells):
            ...
            local_vals = localelement_function_evaluation_P(self.K,self.theta,self.theta_prime,psi_local,P_El)
    ...
    def assemble(self,psi_k):
        ...
        self.lhs = self.J_sat_matrix+self.dt*self.perm_matrix
        self.rhs = self.J_sat_matrix@psi_k +self.sat_matrix_t-self.sat_matrix_k + self.dt*self.source_vector - self.dt*self.gravity_vector 
            
               
class ModL_scheme():
    
    def __init__(self,m,dt,d,g,order,psi, K,theta,theta_prime,f):
        '''
        ...
        dt : Timestep length.
        d : Global to local map.
        g : Geometry from porepy.
        order : Number defining which lagrange element.
        psi : Pressure head.
        K : Permability function.
        theta : Saturation function.
        ...
        self.sat_matrix_t   = np.zeros((d.total_geometric_pts, 1)) 
        self.sat_matrix_k   = np.zeros((d.total_geometric_pts, 1)) 
        self.ML_matrix    = np.zeros((d.total_geometric_pts, d.total_geometric_pts)) 
        self.gravity_vector = np.zeros((d.total_geometric_pts, 1)) 
        self.source_vector  = np.zeros((d.total_geometric_pts, 1))
    ...
    def update_at_iteration(self,psi_k):
        ...
        self.gravity_vector = np.zeros((self.d.total_geometric_pts, 1)) 
        #self.J_perm_matrix    = np.zeros((self.d.total_geometric_pts, self.d.total_geometric_pts))  
        self.ML_matrix    = np.zeros((self.d.total_geometric_pts, self.d.total_geometric_pts)) 
        #self.J_gravity_matrix    = np.zeros((self.d.total_geometric_pts, self.d.total_geometric_pts))  
        ...
        for e in range(self.g.num_cells):
            ...
            for j in range(P_El.num_dofs):
                for i in range(P_El.num_dofs):
                    val=0
                    for k in range(len(Phi)):
                        L = np.amax([local_vals.theta_prime_Q[k]+self.m,2*self.m])
                        val = val + L*quadrature[k][2]*Phi[k][i]*Phi[k][j]
                    #print(val)
                    self.ML_matrix[cn[j]][cn[i]] = self.ML_matrix[cn[j]][cn[i]] + 0.5*np.abs(jac)*val
