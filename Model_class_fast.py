    ...
    def update_at_newtime(self,psi_t,t):
        ...
        for e in range(self.g.num_cells):
            ...
            for j in range(P_El.num_dofs):
                ...
                val2=0
                #_row[n]= cn[j]
                valalt1 = np.sum(local_vals.theta_in_Q*(quadrature[:,2]*Phi[:][j]).reshape(-1,1))
                #print(local_vals.theta_in_Q,'theta', quadrature[:,2],'quad', Phi[:][j],'basis')
                ...
                #print(valalt1, val1)
                self.sat_matrix_t[cn[j]] = self.sat_matrix_t[cn[j]] + 0.5*np.abs(jac)*valalt1
...
class Newton_method_fast():
    ...
    def update_at_iteration(self,psi_k):
        ...
        for e in range(self.g.num_cells):
            cn = self.d.mapping[:,e]
            local_perm, local_gravity, local_saturation_k,local_J_perm, local_J_gravity, local_J_saturation = local_Newton_assembly(e,self.d,psi_k)
            ...
            for k,l in np.ndindex(self.d.element.num_dofs,self.d.element.num_dofs):
                ...
                _data_perm[n] = local_perm[k,l]
                _data_J_perm[n] = local_J_perm[k,l]
                _data_J_sat[n] = local_J_saturation[k,l]
                _data_J_grav[n] = local_J_gravity[k,l]
                n+=1
